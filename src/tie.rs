...
pub fn three_of_kind(com: [i64; 15],plyr: [i64; 15]) -> i64 {
  let c = com.iter()                    // Filter cards that don't have 3 count
             .enumerate()
             .skip(2)
             .filter(|(_,x)| **x == 3)
             .rev();
  let p = plyr.iter()                   // Filter cards that don't have 3 count
              .enumerate()
              .skip(2)
              .filter(|(_,x)| **x == 3)
              .rev();
